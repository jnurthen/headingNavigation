module.exports = function(grunt) {
	grunt.initConfig({
		...
		concat: {
			core: {
				...
				src:   ['src/js/shimIE.js','src/js/<%= pkg.name %>.js'],
		...
		uglify: {
			...
			dist: {
				files: {
					'./compiled/js/<%= pkg.name %>.min.js': ['src/js/shimIE.js', 'src/js/<%= pkg.name %>.js']
				}
			}
		},
		...
		replace: {
			dist: {
				options: {
					variables: {
						'cssContent': '<%= grunt.file.read("src/css/headingNavigation.css") %>'
					}
				},
				files: [{
						expand: true,
	...
	grunt.registerTask('default', ['jshint', 'less', 'concat:core', 'uglify', 'replace', 'concat:gm']);
	grunt.registerTask('all', ['jshint', 'less', 'concat:core', 'uglify', 'replace', 'compress', 'concat:gm']);
